...
function getOriginalText(node) {
    if (!I18N_STATE.originalText.has(node)) {
        I18N_STATE.originalText.set(node, node.nodeValue);
    }
    return I18N_STATE.originalText.get(node);
}
...
function collectTextNodes(root) {
    ...
    while (walker.nextNode()) {
        ...
        if (parent.closest('[data-no-translate="true"], .no-translate')) continue;
        nodes.push(node);
    }
    return nodes;
}
...
function getCachedTranslation(lang, text) {
    if (!I18N_STATE.cache[lang]) I18N_STATE.cache[lang] = {};
    return I18N_STATE.cache[lang][text];
}

function setCachedTranslation(lang, text, translated) {
    if (!I18N_STATE.cache[lang]) I18N_STATE.cache[lang] = {};
    I18N_STATE.cache[lang][text] = translated;
}

async function translateSingle(text, lang) {
    const endpoint = 'https://translate.googleapis.com/translate_a/single';
    const url = `${endpoint}?client=gtx&sl=en&tl=${encodeURIComponent(lang)}&dt=t&q=${encodeURIComponent(text)}`;
    const response = await fetch(url);
    const data = await response.json();
    const translated = (data && data[0]) ? data[0].map(part => part[0]).join('') : text;
    return translated;
}

async function translateBatch(texts, lang) {
    const sentinel = '\n__MUN_TRANSLATE_BREAK__\n';
    const joined = texts.join(sentinel);
    const endpoint = 'https://translate.googleapis.com/translate_a/single';
    const url = `${endpoint}?client=gtx&sl=en&tl=${encodeURIComponent(lang)}&dt=t&q=${encodeURIComponent(joined)}`;
    const response = await fetch(url);
    const data = await response.json();
    const translated = (data && data[0]) ? data[0].map(part => part[0]).join('') : joined;
    const parts = translated.split(sentinel);
    if (parts.length !== texts.length) {
        const results = [];
        for (const text of texts) {
            results.push(await translateSingle(text, lang));
        }
        return results;
    }
    return parts;
    ...
    for (let i = 0; i < pending.length; i += batchSize) {
        ...
        try {
            ...
            translatedBatch.forEach((translated, index) => {
                ...
                setCachedTranslation(lang, original, translated);
                results.set(original, translated);
            });
        } catch (error) {
            batch.forEach(original => results.set(original, original));
        }
    }
...
function collectTranslatableAttributes(root) {
    ...
    const items = [];
    elements.forEach(el => {
        if (el.closest('[data-no-translate="true"], .no-translate')) return;
        attrs.forEach(attr => {
            if (el.hasAttribute(attr)) {
                const value = el.getAttribute(attr);
                if (value) items.push({ el, attr, value });
            }
        });
    });
    return items;
...
async function translatePage(lang) {
    ...
    I18N_STATE.lang = normalizedLang;
    ...
    if (normalizedLang === 'en') {
        ...
        const attrItems = collectTranslatableAttributes(document.body);
        attrItems.forEach(({ el, attr }) => {
            const originalAttr = el.getAttribute(`data-i18n-original-${attr}`);
    ...
    nodes.forEach(node => {
        ...
        if (shouldTranslateText(original)) textsToTranslate.push(original);
    });

    const attrItems = collectTranslatableAttributes(document.body);
    attrItems.forEach(({ el, attr, value }) => {
        if (!el.hasAttribute(`data-i18n-original-${attr}`)) {
            el.setAttribute(`data-i18n-original-${attr}`, value);
        }
        if (shouldTranslateText(value)) textsToTranslate.push(value);
    });

    const translations = await translateTexts([...new Set(textsToTranslate)], normalizedLang);
    nodes.forEach(node => {
        const original = getOriginalText(node);
...
function startTranslationObserver() {
    if (I18N_STATE.observer) return;
    I18N_STATE.observer = new MutationObserver(() => {
        if (I18N_STATE.debounceId) window.clearTimeout(I18N_STATE.debounceId);
        I18N_STATE.debounceId = window.setTimeout(() => {
            if (I18N_STATE.lang !== 'en') {
